...
- 403 = CRM captcha. `handle403()` flips the toolbar button orange ("⚠ CRM Captcha") and sets `_captchaRequired`. Clicking the button calls `openCaptchaPopup()` (popup-blocker-safe because it's a click). The popup polls `/graphql` every 3s with a `{ __typename }` no-op; once status ≠ 403, it auto-closes and the toolbar resets. Manual close also resets.
